...
### 4. デモ用ベースライン `demo/v1.0-base`（初回のみ・クリーンアップPR マージ後・v1.0 確定時）
...
別の方法として `git checkout -B demo/v1.0-base && git push -u origin demo/v1.0-base` でも同様です。

> **レガシー:** 以前は `v1.0-demo-base` タグを使っていました。タグ運用のままなら `demo-reset.sh` 内の参照をタグに合わせてください。
